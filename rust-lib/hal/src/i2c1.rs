...
/// Initialize I2C1
pub fn init(i2c1: &I2C1, master_mode: bool, slave_addr: u32) {
...
pub fn write_txfifo(i2c1: &I2C1, data: &[u8], len: u32)  -> usize {
    ...
    while !tx_em(i2c1) {}
    while !tx_full(i2c1) {
        i2c1.fifo().write(|w| unsafe {w.data().bits(data[written])});
        written += 1;
    }
    return written;
}
...
pub fn master_write(i2c1: &I2C1, addr: u32, data: &[u8], len: u32) {
